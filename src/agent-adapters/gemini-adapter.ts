...
export class GeminiAdapter implements AgentAdapter {
  ...
  private parseOutput(
    ...
    // Gemini JSON wrapper: { session_id, response, stats }
    ...
    try {
      ...
      return {
        ...
        durationMs,
        startedAt,
        error: "Failed to parse Gemini CLI JSON wrapper",
      };
    }

    // No schema enforcement — response is freeform text containing JSON
    ...
    if (typeof responseText !== "string" || !responseText.trim()) {
      return {
        ...
        durationMs,
        startedAt,
        error: "No response text in Gemini output",
      };
    }

    const parseResult = parseFromText(responseText);
